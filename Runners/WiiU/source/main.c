...
#include <stdbool.h>
// nisiquiera se como hacer esto pero bueno 
//saludos especiales a mi papa a mi mama y mi hermano a mis amigos a cristiano ronaldo a messi a jesus a san jose y a mi perro (no tengo perro)
SDL_init_t* g_sdlContext = NULL;
float cam_x = 0;
float cam_y = 0;
float cam_w = 400;
float cam_h = 240;
...
SDL_SpriteSheet* spriteSheet = NULL;
		...
		cJSON_ArrayForEach(layer, layers)
		{
			...
			if (cJSON_IsArray(instances))
			{
				cJSON_ArrayForEach(object, instances)
				{
					...
					Sprite* sp = &sprites[SpriteCount];
					SDL_SpriteFromSheet(&sp->spr, spriteSheet, GetSpriteNumberByName(root, spriteName));
					SDL_SpriteSetPos(&sp->spr, float_x, float_y);
					SDL_SpriteSetScale(&sp->spr, float_scalex, float_scaley);
					SDL_SpriteSetRotation(&sp->spr, float_rot);
...
int main()
{
	...
	gfxInitDefault();
	romfsInit();
	SDL_Init(SDL_DEFAULT_CMDBUF_SIZE);
	SDL_Init(SDL_DEFAULT_MAX_OBJECTS);
	SDL_Prepare();
